# Awesome Amber-Lang projects
<div align="center">
    <img src="https://github.com/amber-lang/amber/blob/staging/assets/amber.png" alt="amber logo" width="250" />
</div>

[Join our Discord](https://discord.com/invite/cjHjxbsDvZ) - [Documentation website](https://docs.amber-lang.com/)

We look for projects with a Readme!

## Amber tools

* [ArjixWasTaken/amethyst](https://github.com/ArjixWasTaken/amethyst) - 🔮 Amethyst - A version manager, and project manager for Amber.
* https://github.com/amber-lang/amber/tree/staging/scripts

## Distro

* [nixie-dev/nixie](https://github.com/nixie-dev/nixie) - Nixie is a lightweight solution to ship the Nix package manager together with a project repository, regardless of host architecture.

## Utils

* [krissh-wtf/amber-httpclient](https://github.com/krissh-wtf/amber-httpclient) - a POSIX-compliant http client library for bash/amber without using external dependencies (curl, wget, netcat, etc...)
* [LSP-Installer](https://github.com/Mte90/My-Scripts/tree/master/dev/lsp-installer) - A simple way to install various LSP and testing various Amber features.

## Collections

* [UrbanCoffee/amber-projects](https://github.com/UrbanCoffee/amber-projects) - A collection of scripts and programs in the Amber programming language.
* [rbtylee/AmberResources](https://github.com/rbtylee/AmberResources) - A collection of Amber libraries and other resources.
